...
let GSSLocalStoreChangeTableFilename = "GSSLocalStoreChangeTable.sqlite"

class GSSChangeTableManager: NSObject {
    ...
    private var storeURL: NSURL {
        return NSURL.gss_applicationDocumentsDirectory().URLByAppendingPathComponent("GSSChangeTableManagerCoreData.sqlite")
    }
    ...
    private func logChangedManagedObjects(objects: Set<NSManagedObject>, changeType: GSSChangeTableChangeType) {
        /**
        ...
        */
        ...
        for obj in objects {
            ...
            fetchRequest.predicate = NSPredicate(format: "%K == %@", GSSLocalRecordIDAttributeName, obj.valueForKey(GSSLocalRecordIDAttributeName) as! String)
            ...
            if results.isEmpty {
                ...
                changeTableEntry = NSEntityDescription.insertNewObjectForEntityForName(GSSChangeTableEntityName, inManagedObjectContext: self.changeTableContext)
            }
            else {
                ...
                let raw = changeTableEntry?.valueForKey(GSSLocalRecordChangeTypeAttributeName) as! NSNumber
                
                if let currentChangeType = GSSChangeTableChangeType(number: raw) {
            ...
            case .UpdateIncludingChangeType:
                ...
                changeTableEntry?.setValue(changeType.toNumber(), forKey: GSSLocalRecordChangeTypeAttributeName)
    ...
    lazy var model: NSManagedObjectModel = {
        ...
        changeTypeAttribute.name = GSSLocalRecordChangeTypeAttributeName
