...
typealias MFTSyncOperationConflictResolutionBlock = (serverRecord: CKRecord, clientRecord: CKRecord, ancestorRecord: CKRecord) -> CKRecord

enum MFTSyncOperationConflictResolutionPolicy {
...
class MFTSyncOperation: NSOperation {
     ...
     */
    var conflicResolutionPolicy = MFTSyncOperationConflictResolutionPolicy.KeepServerRecord
     ...
     */
    var perRecordConflictResolutionBlock: MFTSyncOperationConflictResolutionBlock?
    ...
    */
    init(persistentStoreCoordinator: NSPersistentStoreCoordinator, changeTableManager: MFTChangeTableManager, mainContext: NSManagedObjectContext) {
                        ...
                        for (recordID, recordError) in partialErrorsByRecordID {
                            if recordError.code == CKErrorCode.ServerRecordChanged.rawValue {
                                ...
                                case .KeepNewerRecord:
                                    ...
                                    let serverRecordModificationDate = server[MFTLocalRecordModificationDateAttributeName] as! NSDate
                                    let clientRecordModificationDate = client[MFTLocalRecordModificationDateAttributeName] as! NSDate
    ...
    func fetchRecordChangesUsingServerChangeToken(serverChangeToken: CKServerChangeToken?) throws -> (insertedOrUpdated: [CKRecord], deleted: [CKRecordID], cursor: MFTFetchCursor) {
        var insertedOrUpdatedCKRecords = [CKRecord]()
        var deletedCKRecodsIDs = [CKRecordID]()
        var cursor = MFTFetchCursor(moreComing: false, serverChangeToken: serverChangeToken)
        var error: NSError?
        
        let recordZoneID = CKRecordZoneID(zoneName: MFTCloudKitSyncManagerZoneName, ownerName: CKOwnerDefaultName)
        ...
        fetchRecordChangesOperation.fetchRecordChangesCompletionBlock = {(serverChangeToken: CKServerChangeToken?, clientChangeTokenData: NSData?, operationError: NSError?) -> Void in
            ...
            cursor = MFTFetchCursor(moreComing: fetchRecordChangesOperation.moreComing, serverChangeToken: serverChangeToken)
        }
    ...
    func insertedOrUpdatedCKRecordsByRecordID() -> [CKRecordID : CKRecord] {
        ...
        changeTablePrivateContext.performBlockAndWait({ () -> Void in
            ...
            let insertChangeType = MFTChangeTableChangeType.Insert.toNumber()
            let updateChangeType = MFTChangeTableChangeType.Update.toNumber()
            let predicate = NSPredicate(format: "%K == %@ || %K == %@", MFTLocalRecordChangeTypeAttributeName, insertChangeType, MFTLocalRecordChangeTypeAttributeName, updateChangeType)
    ...
    func deletedCKRecordIDs() -> [CKRecordID] {
        ...
        changeTablePrivateContext.performBlockAndWait { () -> Void in
            let deleteChangeType = MFTChangeTableChangeType.Delete.toNumber()
            
            let fetchRequest = NSFetchRequest(entityName: MFTChangeTableEntityName)
            fetchRequest.predicate = NSPredicate(format: "%K == %@", MFTLocalRecordChangeTypeAttributeName, deleteChangeType)
    ...
    func managedObjectForChangeTableEntry(changeTableEntry: NSManagedObject) -> NSManagedObject? {
        ...
        mainPrivateContext.performBlockAndWait { () -> Void in
            let fetchRequest = NSFetchRequest(entityName: changeTableEntry.valueForKey(MFTLocalRecordEntityNameAttributeName) as! String)
            fetchRequest.predicate = NSPredicate(format: "%K == %@", MFTLocalRecordIDAttributeName, changeTableEntry.valueForKey(MFTLocalRecordIDAttributeName) as! String)
    ...
    func wipeChangeTable() {
        ...
        let deleteRequest = NSBatchDeleteRequest(fetchRequest: NSFetchRequest(entityName: MFTChangeTableEntityName))
    ...
    func processInsertedOrUpdatedCKRecords(insertedOrUpdated: [CKRecord], deletedRecordIDs: [CKRecordID]) throws {
        ...
        if let d = deduplicator {
            let deduplicationManager = MFTDeduplicationManager()
    ...
    func processInsertedOrUpdatedCKRecords(records: [CKRecord]) {
        mainPrivateContext.performBlockAndWait { () -> Void in
            var referencesByManagedObject = [NSManagedObject : [String : MFTReference]]()
            ...
            for record in records {
                ...
                fetchRequest.predicate = NSPredicate(format: "%K == %@", MFTLocalRecordIDAttributeName, record.valueForKey(MFTLocalRecordIDAttributeName) as! String)
                fetchRequest.fetchLimit = 1
            ...
            for (object, references) in referencesByManagedObject {
                for (name, ref) in references {
                    ...
                    fetchRequest.predicate = NSPredicate(format: "%K == %@", MFTLocalRecordIDAttributeName, destinationRecordID)
                    fetchRequest.fetchLimit = 1
    ...
    func processDeletedRecordIDs(recordIDs: [CKRecordID]) {
        if recordIDs.count > 0 {
            mainPrivateContext.performBlockAndWait { () -> Void in
                let predicate = NSPredicate(format: "%K IN $RECORD_IDs", MFTLocalRecordIDAttributeName)
